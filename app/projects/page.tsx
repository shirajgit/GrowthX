...
function Ring({ value, color }: { value: number; color: string }) {
  ...
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" className="flex-shrink-0">
      <circle cx="24" cy="24" r={r} fill="none" style={{ stroke: "var(--border-strong)" }} strokeWidth="4" />
    ...
    <div className="min-h-screen text-white p-4 sm:p-6" style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: "var(--bg-grad)" }}>
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[0, 1, 2].map((i) => <div key={i} className="h-44 rounded-2xl animate-pulse" style={{ background: "var(--surface)" }} />)}
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <AnimatePresence>
            {projects.map((project, i) => {
              ...
              return (
                <motion.div key={project._id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                  ...
                  className="relative rounded-2xl p-5 group overflow-hidden" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                        ...
                        {project.tech && (
                          <div className="flex flex-wrap gap-1">
                            {project.tech.split(",").filter(Boolean).slice(0, 3).map((t) => (
                              <span key={t} className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: "var(--surface-hover)", color: "#94a3b8" }}>{t.trim()}</span>
                            ))}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="w-full h-1.5 rounded-full overflow-hidden mb-4" style={{ background: "var(--surface-hover)" }}>
                    ...
                    {editingId === project._id ? (
                      <div className="flex items-center gap-2">
                        <input type="number" value={editProgress} onChange={(e) => setEditProgress(e.target.value)} min="0" max="100"
                          className="w-16 px-2 py-1 text-xs rounded-lg outline-none" style={{ background: "var(--border-strong)", border: "1px solid var(--border-strong)", color: "var(--text)" }} />
