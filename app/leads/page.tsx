...
import { Plus, Trash2, Search, X, Flame, Users, TrendingUp, Percent, MessageCircle, Pencil } from "lucide-react";
...
export default function LeadsPage() {
  ...
  const saveLead = async () => {
    ...
    await fetch(`/api/leads/${editingLead._id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editingLead) });
    ...
    <div className="min-h-screen text-white p-4 sm:p-6" style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: "linear-gradient(160deg, #080808 0%, #0f0f13 50%, #0a0a10 100%)" }}>
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] tracking-[0.25em] uppercase text-gray-600 font-semibold">Pipeline</p>
            <h1 className="text-2xl font-bold tracking-tight leading-none mt-0.5">Leads</h1>
          </div>
        </div>
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setShowForm((s) => !s)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold" style={{ background: `${ACCENT}1f`, border: `1px solid ${ACCENT}50`, color: "#fca5a5" }}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <StatCard label="Total" value={leads.length} color="#a78bfa" icon={Users} delay={0.04} />
        <StatCard label="Open" value={open} color="#60a5fa" icon={Flame} delay={0.08} />
        <StatCard label="Converted" value={converted} color="#22c55e" icon={TrendingUp} delay={0.12} />
        <StatCard label="Conv. Rate" value={`${rate}%`} color={ACCENT} icon={Percent} delay={0.16} />
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={addLead}
              className="mt-3 flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold" style={{ background: `linear-gradient(135deg, ${ACCENT}33, ${ACCENT}1a)`, border: `1px solid ${ACCENT}50`, color: "#fca5a5" }}>
                ...
                {filtered.map((lead, i) => {
                  ...
                  return (
                    <motion.tr key={lead._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ delay: i * 0.03 }}
                      ...
                      <td className="px-5 py-4">
                        <select value={lead.status} onChange={(e) => updateStatus(lead._id, e.target.value)} className="px-2.5 py-1 rounded-full text-xs font-semibold outline-none cursor-pointer" style={{ background: sc.bg, border: `1px solid ${sc.border}`, color: sc.color }}>
                          {Object.entries(statusConfig).map(([key, s]) => <option key={key} value={key}>{s.label}</option>)}
                        </select>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setEditingLead(lead)} className="text-gray-500 hover:text-blue-400"><Pencil size={14} /></button>
                          <a href={`https://wa.me/${lead.phone || ""}`} target="_blank" className="text-gray-500 hover:text-green-400"><MessageCircle size={14} /></a>
                          <button onClick={() => deleteLead(lead._id)} className="text-gray-500 hover:text-red-400"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </motion.tr>
                  );
            ...
            <motion.div key={lead._id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="font-semibold">{lead.name}</p><p className="text-xs text-gray-500">{lead.company} · {lead.work}</p></div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <select value={lead.status} onChange={(e) => updateStatus(lead._id, e.target.value)} className="px-2.5 py-1 rounded-full text-xs font-semibold outline-none" style={{ background: sc.bg, border: `1px solid ${sc.border}`, color: sc.color }}>
                  {Object.entries(statusConfig).map(([key, s]) => <option key={key} value={key}>{s.label}</option>)}
                </select>
                <a href={`https://wa.me/${lead.phone || ""}`} target="_blank" className="flex items-center gap-1 text-xs text-green-400"><MessageCircle size={13} /> WhatsApp</a>
              </div>
            </motion.div>
          );
        })}
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center mt-20 text-gray-600"><Flame size={32} className="mx-auto mb-3 opacity-30" /><p className="text-sm">No leads found</p></div>
      )}
