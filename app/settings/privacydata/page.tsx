...
import {
  ...
  Globe, Check, ChevronRight, AlertTriangle,
  Database, FileText, X, Loader2,
} from "lucide-react";

type ToggleSetting = { id: string; label: string; sub: string; value: boolean };

const dataCategories = [
  { key: "tasksNotes", icon: FileText, label: "Tasks & Notes", sub: "All tasks, notes, and their timestamps", color: "#34d399" },
  { key: "projectsJobs", icon: Server, label: "Projects & Jobs", sub: "Project names, progress, job applications", color: "#3b82f6" },
  { key: "clientsLeads", icon: Globe, label: "Clients & Leads", sub: "Contact info, companies, statuses", color: "#ec4899" },
  { key: "settings", icon: Database, label: "Settings & Profile", sub: "Display name, theme, preferences", color: "#a78bfa" },
];

const privacyPolicySections = [
  { title: "What we collect", body: "GrowthX collects only the data you explicitly enter: tasks, projects, notes, clients, leads, job applications, and your account preferences. We do not collect browsing history, device fingerprints, or third-party behavioral data." },
  { title: "How we use it", body: "Your data is used exclusively to power your GrowthX workspace and generate AI-driven insights within the app. We do not sell, rent, or share your data with advertising networks or data brokers." },
  { title: "Data storage", body: "All data is stored in MongoDB Atlas on servers in your region. Data is encrypted at rest (AES-256) and in transit (TLS 1.3). Backups are taken daily and retained for 30 days." },
  { title: "Authentication", body: "Authentication is handled by Clerk, a SOC 2 Type II certified provider. GrowthX never stores your password. Session tokens are short-lived and rotated automatically." },
  { title: "AI & third parties", body: "AI features use the Anthropic API. Only the specific prompt and minimal workspace context needed to answer your query is sent. No conversation history is retained by Anthropic beyond the current request." },
  { title: "Your rights", body: "You have the right to access, correct, export, or delete all your data at any time. Submit a data request or delete your account directly from this page." },
];

function ToggleRow({ setting, onChange }: { setting: ToggleSetting; onChange: (id: string, val: boolean) => void }) {
  return (
...
export default function PrivacyDataPage() {
  ...
  // Load persisted privacy preferences
  useEffect(() => {
    fetch("/api/profile")
      .then((r) => (r.ok ? r.json() : null))
      .then((profile) => {
        if (!profile?.privacy) return;
        setPrivacySettings((prev) => prev.map((s) => (s.id in profile.privacy ? { ...s, value: !!profile.privacy[s.id] } : s)));
      })
      .catch(() => {});
  }, []);

  // Real item counts instead of fake sizes
  useEffect(() => {
    const j = (u: string) => fetch(u).then((r) => r.json()).catch(() => []);
    Promise.all([j("/api/tasks"), j("/api/notes"), j("/api/projects"), j("/api/jobs"), j("/api/clients"), j("/api/leads")])
      .then(([tasks, notes, projects, jobs, clients, leads]) => {
        setCounts({
          tasksNotes: (tasks?.length || 0) + (notes?.length || 0),
          projectsJobs: (projects?.length || 0) + (jobs?.length || 0),
          clientsLeads: (clients?.length || 0) + (leads?.length || 0),
          settings: 1,
        });
      });
  }, []);

  // Optimistic toggle — update UI immediately, persist, revert on failure
  const toggleSetting = async (id: string, val: boolean) => {
    setSaveError(null);
    const prevSettings = privacySettings;
    const next = prevSettings.map((s) => (s.id === id ? { ...s, value: val } : s));
    setPrivacySettings(next);

    const privacy = next.reduce<Record<string, boolean>>((acc, s) => ({ ...acc, [s.id]: s.value }), {});
    try {
      ...
      setPrivacySettings(prevSettings); // revert
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const exportData = { exported_at: new Date().toISOString(), app: "GrowthX", data: { tasks, projects, jobs, clients, leads, notes } };
      const blob = new Blob([JSON.stringify(exportData, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `growthx-export-${new Date().toISOString().split("T")[0]}.json`; a.click();
  ...
  // Wired delete — calls your account endpoint, then redirects.
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleting(false);
      setSaveError("Couldn't delete your account. Please try again.");
    }
  };
  ...
  return (
    <div className="min-h-screen text-white" style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: "linear-gradient(160deg, #080808 0%, #0f0f13 50%, #0a0a10 100%)" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 pb-24">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-9">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.35), rgba(139,92,246,0.15))", border: "1px solid rgba(99,102,241,0.4)", boxShadow: "0 0 24px rgba(99,102,241,0.18)" }}>
              <Shield size={20} style={{ color: "#818cf8" }} />
            </div>
            <div>
              <p className="text-[11px] tracking-[0.25em] uppercase text-gray-600 font-semibold">Legal & Security</p>
              <h1 className="text-2xl font-bold tracking-tight leading-none mt-0.5">Privacy & Data</h1>
            </div>
          </div>
          <p className="text-gray-400 text-sm mt-3">Control your data, privacy preferences, and account deletion.</p>
        </motion.div>
          ...
          <AnimatePresence>
            {saveError && (
              <motion.p role="alert" initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="text-xs mt-2 px-1 flex items-center gap-1.5" style={{ color: "#f87171" }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="p-4 grid grid-cols-2 gap-2">
              {dataCategories.map((cat, i) => {
                ...
                return (
                  <motion.div key={cat.label} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 + i * 0.06 }} whileHover={{ y: -2 }}
                    className="flex items-start gap-3 p-3 rounded-xl" style={{ background: `${cat.color}0a`, border: `1px solid ${cat.color}18` }}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400 leading-snug">{cat.sub}</p>
                      <p className="text-[10px] mt-1 font-medium" style={{ color: cat.color }}>
                        {counts === null ? "Loading…" : cat.key === "settings" ? "Account synced" : `${n} item${n === 1 ? "" : "s"}`}
                      </p>
                    </div>
                  </motion.div>
                );
            ...
            {[
              ...
              return (
                <motion.div key={item.label} whileHover={{ x: 2 }} className="flex items-center gap-4 px-4 py-3.5" style={{ borderTop: i > 0 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                  ...
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0" style={{ background: `${item.color}15`, border: `1px solid ${item.color}25`, color: item.color }}>{item.badge}</span>
                </motion.div>
              );
            })}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.38 }} className="mb-5">
          ...
          <div className="rounded-2xl p-5 space-y-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div className="pt-2">
              <button className="flex items-center gap-1.5 text-xs font-semibold" style={{ color: "#818cf8" }}>Read full Privacy Policy <ChevronRight size={12} /></button>
            </div>
          </div>
        </motion.div>
          ...
          <div className="rounded-2xl p-5" style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.15)" }}>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">This permanently deletes your account, all tasks, projects, notes, clients, leads, jobs, and settings. This action cannot be undone.</p>
              </div>
            </div>
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} onClick={() => setShowDeleteModal(true)} aria-label="Delete my account and all data"
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all outline-none focus-visible:ring-2 focus-visible:ring-red-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a10]" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", color: "#ef4444" }}>
              <Trash2 size={14} /> Delete My Account
            </motion.button>
          </div>
        </motion.div>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="text-center text-[11px] text-gray-700 mt-10 tracking-wide">
          GrowthX · GDPR & CCPA compliant
        </motion.p>
      </div>
        ...
        {showDeleteModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4"
            ...
            <motion.div role="dialog" aria-modal="true" aria-labelledby="delete-modal-title" initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.93, opacity: 0 }} transition={{ duration: 0.2 }}
              ...
              <h2 id="delete-modal-title" className="text-lg font-bold text-white mb-1">Delete everything?</h2>
              <p className="text-sm text-gray-400 mb-5 leading-relaxed">Type <span className="font-mono text-red-400 text-xs px-1.5 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.1)" }}>DELETE</span> to confirm. This is permanent.</p>
